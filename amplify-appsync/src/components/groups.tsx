    ...
    const InsertGroup = async () => {
        if (getGrp) {
          const getGroup: any = await API.graphql(
            graphqlOperation(createGroup, { input: {name: getGrp} })
          );
          props.Groups.push({ label: getGroup.data.createGroup.name, value:  getGroup.data.createGroup.id });
            setGroupRes(false);
            setGrp(undefined);
        }
      };
    ...
    <div className='form-group mt-3'>
        ...
        <div className='row col-md-12'>
          ...
          <Multiselect
            ...
            isObject={true}
            onRemove={(removed) => { props.setGroups(removed); setGrp(undefined); }}
            onSearch={handleSearch}
            onSelect={(add) => { props.setGroups(add); setGrp(undefined);}}
