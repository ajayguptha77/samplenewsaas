...
import { createExternalKeywords } from '../../graphql/mutations';
import { API, graphqlOperation } from 'aws-amplify';
import { listExternalKeywords } from '../../graphql/queries';
...
function ExternalKeywordsComponent(props: any) {
    ...
    useEffect(() => {
        async function groups() {
          ...
          try {
            const AllGroups = await API.graphql(graphqlOperation(listExternalKeywords)) as any;
            setGroupData(AllGroups.data.listExternalKeywords.items)
            setGroupRes(true);
          } catch (err) {
    ...
    const InsertGroup = async () => {
        if (getGrp) {
          const getGroup: any = await API.graphql(
            graphqlOperation(createExternalKeywords, { input: {name: getGrp} })
          );
          props.ExternalKeywords.push({ label: getGroup.data.createExternalKeywords.name, value:  getGroup.data.createExternalKeywords.id });
            setGroupRes(false);
            setGrp(undefined);
        }
      };
    ...
    <div className='form-group mt-3'>
        ...
        <div className='row col-md-12'>
          ...
          <Multiselect
            ...
            isObject={true}
            onRemove={(removed) => { props.setExternalKeywords(removed); setGrp(undefined); }}
            onSearch={handleSearch}
            onSelect={(add) => { props.setExternalKeywords(add); setGrp(undefined);}}
...
export default ExternalKeywordsComponent;
